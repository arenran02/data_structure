...
// 노드 생성
Node* createNode(int index, int move) {
...
// 원형 리스트 생성
Node* createCircularList(int n, int moves[]) {
    Node* head = NULL;
    Node* tail = NULL;

    for (int i = 0; i < n; i++) {
        Node* newNode = createNode(i + 1, moves[i]);
...
void popBalloons(Node* head) {
    ...
    while (head) {
        ...
        current = removeNode(&head, current);
...
int main() {
    ...
    Node* head = createCircularList(N, moves);
    popBalloons(head);
